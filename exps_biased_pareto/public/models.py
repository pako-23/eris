...
def train_with_opacus(model, device, train_loader, optimizer, criterion, sigma, epochs=1, client_id=None, sensitivity=1): 
    model.train()
    ...
    print(f"Client {client_id} - Epoch {epoch+1}/{epochs} | Loss: {avg_loss:.4f} | ε = {cur_epsilon:.2f} (δ={cfg.delta}) | Sensitivity: {sensitivity}")
...
def train_llm_with_opacus(model, device, train_data, training_args, sigma, delta, client_id=None, sensitivity=1): 
    ...
    model, optimizer, train_loader = privacy_engine.make_private(
                        ...
                        data_loader=train_loader,
                        noise_multiplier=sigma,
                        max_grad_norm=sensitivity, # it was commented before

...
def save_results_xlsx(results: dict, xlsx_path: str, max_rounds: int | None = None):
    """
...
def main():
    ...
    parser.add_argument("--model_name", type=str, default="EleutherAI/gpt-neo-1.3B", #default="gpt2-xl", gpt2 for testing
    ...
    parser.add_argument("--learning_rate", type=float, default=1e-5) # try smaller e.g., 1e-5
    ...
    parser.add_argument("--client_training_samples", type=int, default=128, help="Number of training samples per client.")
    ...
    if datasets_exist:
        ...
        for cid, (tr, va) in enumerate(zip(client_trains, client_vals)):
            print(f"Client {cid} - Round {rnd}: train={len(tr)}, val={len(va)}")
    ...
    # save figure
    fig_path = os.path.join(args.output_dir, f"images/training_mia_trends_FedAvg+DP_F{args.fold}.pdf")

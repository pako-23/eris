...
# This script is used to run the federated learning experiments for the FedAvg framework. Based on the
# config file, it can be used to run Pruning-based FL, Local Differential Privacy-based FL.
...
# of the added differential privacy noise (or the pruning rate) to draw the Pareto front.
...
for exp_n in $(seq 0 3); do
    ...
    for scaling_dp in $(seq 0 10); do
        ...
        if [ $k_folds -gt 1 ]; then
            ...
            cd ../public
            python average_results.py --strategy "fedavg" --dataset $dataset_name --exp_n $exp_n --scaling_dp $scaling_dp
            sleep 1
        fi

...
# This script is used to run the federated learning experiments for the SoteriaFL framework (with LLMs).
...
for exp_n in $(seq 0 5); do
    ...
    if [ $k_folds -gt 1 ]; then
        ...
        cd ../public
        python average_results.py --strategy "soterafl_llm" --dataset $dataset_name --exp_n $exp_n
        sleep 1
    fi

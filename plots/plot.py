...
# ROG Data
ssim_rog_mean = np.array([0.8746, 0.875, 0.874, 0.8731, 0.8739, 0.8744, 0.8735, 0.8731, 0.8611, 0.6755, 0.3113, 0.2841, 0.2724])
ssim_rog_std = np.array([0.0715, 0.073, 0.0731, 0.0756, 0.0728, 0.0709, 0.0702, 0.0724, 0.0761, 0.1776, 0.1236, 0.094, 0.0873]) / np.sqrt(5)
lpips_rog_mean = np.array([0.0306, 0.0298, 0.0299, 0.0306, 0.0314, 0.0298, 0.0297, 0.0308, 0.0417, 0.0898, 0.1595, 0.1816, 0.1869])
lpips_rog_std = np.array([0.0216, 0.0208, 0.0205, 0.0209, 0.023, 0.0208, 0.0205, 0.0217, 0.0266, 0.0526, 0.0713, 0.0836, 0.0807]) / np.sqrt(5)
...
lpips_random_withmodel = 0.1868
lpips_random_withmodel_std = 0.0779 / np.sqrt(5)
...
ax1.errorbar(x_positions, ssim_rog_mean, yerr=ssim_rog_std, fmt='^-',
                capsize=5, label='ROG', color='red', markersize=4, linewidth=1.2)
...
ax2.errorbar(x_positions, lpips_idlg_mean, yerr=lpips_idlg_std, fmt='s-', 
             capsize=5, label='iDLG', color='blue', markersize=4, linewidth=1.2)
ax2.errorbar(x_positions, lpips_rog_mean, yerr=lpips_rog_std, fmt='^-',
                capsize=5, label='ROG', color='red', markersize=4, linewidth=1.2)
# Plot random baseline as a horizontal dashed line with a shaded error band
ax2.axhline(lpips_random, color='green', linestyle='--', linewidth=1.2, label='Random Image')
ax2.axhspan(lpips_random - lpips_random_std, lpips_random + lpips_random_std, 
            facecolor='green', alpha=0.2)
ax2.axhline(lpips_random_withmodel, color='purple', linestyle='--', linewidth=1.2, label='Random Gradients')
ax2.axhspan(lpips_random_withmodel - lpips_random_withmodel_std, lpips_random_withmodel + lpips_random_withmodel_std,
...
# ROG Data
ssim_rog_mean = np.array([0.7764, 0.7776, 0.7773, 0.7768, 0.7752, 0.7497, 0.7113, 0.6189, 0.3836, 0.1476, 0.0992, 0.0893, 0.0875])
ssim_rog_std = np.array([0.097, 0.0957, 0.0958, 0.0961, 0.0962, 0.0998, 0.1027, 0.1042, 0.0718, 0.0423, 0.0494, 0.052, 0.055]) / np.sqrt(5)
lpips_rog_mean = np.array([0.0812, 0.08, 0.0802, 0.0797, 0.0806, 0.0861, 0.0936, 0.1169, 0.1852,     0.2858, 0.3573, 0.3764, 0.3832])
lpips_rog_std = np.array([0.021, 0.0207, 0.0208, 0.0204, 0.021, 0.0223, 0.0249, 0.0296, 0.0539, 0.0621, 0.0675, 0.075, 0.0748]) / np.sqrt(5)

# Random baseline (common to both methods)
ssim_random = 0.008068
ssim_random_std = 0.011617 / np.sqrt(5)
# lpips_random_oldcode = 0.415879
# lpips_random_newcode = 0.3922
# diff = lpips_random_oldcode - lpips_random_newcode
lpips_random = 0.415879
lpips_random_std = 0.072017 / np.sqrt(5)
lpips_random_withmodel = 0.3866
lpips_random_withmodel_std = 0.0754 / np.sqrt(5)

# correction rog bias metric
# lpips_rog_mean = lpips_rog_mean + diff
...
# ROG Data
ssim_rog_mean = np.array([0.822, 0.8221, 0.8222, 0.8222, 0.8212, 0.8146, 0.8083, 0.7937, 0.7371, 0.5286, 0.2437, 0.1316, 0.0965])
ssim_rog_std = np.array([0.0616, 0.0616, 0.0617, 0.0618, 0.062, 0.0622, 0.0645, 0.0679, 0.0755, 0.1049, 0.1435, 0.1484, 0.1501]) / np.sqrt(5)
lpips_rog_mean = np.array([0.075, 0.0753, 0.0751, 0.0751, 0.0751, 0.08, 0.0816, 0.0877, 0.134, 0.1895, 0.2485, 0.2603, 0.2617])
lpips_rog_std = np.array([0.0379, 0.0382, 0.0382, 0.0384, 0.0384, 0.0398, 0.0394, 0.0386, 0.0409, 0.0438, 0.0451, 0.0479, 0.0444]) / np.sqrt(5)

# Random baseline (common to both methods)
ssim_random = 0.006688
ssim_random_std = 0.011079 / np.sqrt(5)
lpips_random = 0.472369 # 0.4313 +- 0.0673
lpips_random_new = 0.4213
lpips_random_new_std = 0.0673
lpips_random_std = 0.063956 / np.sqrt(5)
# lpips_random_withmodel = 0.2631 +- 0.048
lpips_random_withmodel = 0.2631
lpips_random_withmodel_std = 0.048 / np.sqrt(5)

diff = lpips_random - lpips_random_new
lpips_rog_mean = lpips_rog_mean + diff
lpips_random_withmodel = lpips_random_withmodel + diff
...
lpips_idlg_std_mnist = np.array([0.136978, 0.141556, 0.13649, 0.111773, 
                           0.089535, 0.085258, 0.087831, 0.093293, 0.085115, 0.087164, 0.089007]) / np.sqrt(5)
lpips_rog_mean_mnist = np.array([0.0306, 0.0306, 0.0314, 0.0298, 0.0297, 0.0308, 0.0417, 0.0898, 0.1595, 0.1816, 0.1869])
lpips_rog_std_mnist = np.array([0.0216, 0.0209, 0.023, 0.0208, 0.0205, 0.0217, 0.0266, 0.0526, 0.0713, 0.0836, 0.0807]) / np.sqrt(5)

lpips_random_mnist = 0.566315
lpips_random_std_mnist = 0.097331 / np.sqrt(5)
lpips_random_withmodel_mnist = 0.1868
lpips_random_withmodel_std_mnist = 0.0779 / np.sqrt(5)
...
lpips_idlg_std_cifar = np.array([0.137549, 0.107466, 0.075881, 0.074989, 
                           0.07248, 0.071449, 0.072945, 0.074391271, 0.072059309, 0.073025471, 0.070792152]) / np.sqrt(5)
lpips_rog_mean = np.array([0.0812, 0.0797, 0.0806, 0.0861, 0.0936, 0.1169, 0.1852, 0.2858, 0.3573, 0.3764, 0.3832])
lpips_rog_std = np.array([0.021, 0.0204, 0.021, 0.0223, 0.0249, 0.0296, 0.0539, 0.0621, 0.0675, 0.075, 0.0748]) / np.sqrt(5)

lpips_random_cifar = 0.415879
lpips_random_std_cifar = 0.072017 / np.sqrt(5)
lpips_random_withmodel_cifar = 0.3866
lpips_random_withmodel_std_cifar = 0.0754 / np.sqrt(5)
...
lpips_rog_mean_lfw = np.array([0.075, 0.0751, 0.0751, 0.08, 0.0816, 0.0877, 0.134, 0.1895, 0.2485, 0.2603, 0.2617])
lpips_rog_std_lfw = np.array([0.0379, 0.0384, 0.0384, 0.0398, 0.0394, 0.0386, 0.0409, 0.0438, 0.0451, 0.0479, 0.0444]) / np.sqrt(5)
...
# bias correction
lpips_random_new_lfw = 0.4213
lpips_random_new_std_lfw = 0.0673
diff = lpips_random_lfw - lpips_random_new_lfw
lpips_rog_mean_lfw = lpips_rog_mean_lfw + diff
lpips_random_withmodel = lpips_random_withmodel + diff
...
ax1.errorbar(x_positions, lpips_rog_mean_mnist, yerr=lpips_rog_std_mnist, fmt='^-',
                capsize=5, label='_nolegend_', color='darkgreen', markersize=4, linewidth=1.2, alpha=0.3)
...
ax1.plot(x_positions, lpips_rog_mean_mnist, '^-', color='darkgreen', markersize=4, linewidth=1.2, label='ROG')
# Plot random baseline as a horizontal dashed line with a shaded error band
ax1.axhline(lpips_random_mnist, color='green', linestyle='--', linewidth=1.2, label='Random Image')
...
ax1.axhline(lpips_random_withmodel_mnist, color='purple', linestyle='--', linewidth=1.2, label='Random Gradients')
ax1.axhspan(lpips_random_withmodel_mnist - lpips_random_withmodel_std_mnist, lpips_random_withmodel_mnist + lpips_random_withmodel_std_mnist,
...
ax2.plot(x_positions, lpips_rog_mean, '^-', color='darkgreen', markersize=4, linewidth=1.2, label='ROG')
# Plot random baseline as a horizontal dashed line with a shaded error band
ax2.axhline(lpips_random_cifar, color='green', linestyle='--', linewidth=1.2, label='Random Image')
ax2.axhspan(lpips_random_cifar - lpips_random_std_cifar, lpips_random_cifar + lpips_random_std_cifar, 
            facecolor='green', alpha=0.15)
ax2.axhline(lpips_random_withmodel_cifar, color='purple', linestyle='--', linewidth=1.2, label='Random Gradients')
ax2.axhspan(lpips_random_withmodel_cifar - lpips_random_withmodel_std_cifar, lpips_random_withmodel_cifar + lpips_random_withmodel_std_cifar,
            facecolor='purple', alpha=0.15)
...
ax3.errorbar(x_positions, lpips_idlg_mean_lfw, yerr=lpips_idlg_std_lfw, fmt='s-',
                capsize=5, label='_nolegend_', color='blue', markersize=4, linewidth=1.2, alpha=0.3)
ax3.errorbar(x_positions, lpips_rog_mean_lfw, yerr=lpips_rog_std_lfw, fmt='^-',
                capsize=5, label='_nolegend_', color='darkgreen', markersize=4, linewidth=1.2, alpha=0.3)
...
ax3.plot(x_positions, lpips_rog_mean_lfw, '^-', color='darkgreen', markersize=4, linewidth=1.2, label='ROG')

# Plot random baseline as a horizontal dashed line with a shaded error band
ax3.axhline(lpips_random_lfw, color='green', linestyle='--', linewidth=1.2, label='Random Image')
ax3.axhspan(lpips_random_lfw - lpips_random_std_lfw, lpips_random_lfw + lpips_random_std_lfw, 
            facecolor='green', alpha=0.15)
ax3.axhline(lpips_random_withmodel, color='purple', linestyle='--', linewidth=1.2, label='Random Gradients')
ax3.axhspan(lpips_random_withmodel - lpips_random_withmodel_std, lpips_random_withmodel + lpips_random_withmodel_std,
            facecolor='purple', alpha=0.15)
...
for ax in [ax1, ax2, ax3]:
    ...
    ax.axvspan(left_boundary, right_boundary, facecolor='lightgrey', alpha=0.35, zorder=0, label='ERIS')

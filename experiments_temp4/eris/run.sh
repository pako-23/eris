...
for exp_n in $(seq 0 0); do
    ...
    for fold in $(seq 1 $k_folds); do
        ...
        # Start training
        ./coordinator4.py --dataset_name "$dataset_name" --exp_n "$exp_n" &
        # sleep 0.5
        sleep 3

        for i in $(seq 1 "$n_clients"); do
        if [ "$i" -le "$aggregators" ]; then
            ./client_eris4.py --aggregator                   \
                --id "$i"                      \
        ...
        while pgrep -f ./client_eris4.py >/dev/null; do
            sleep 5
        done

        pkill -9 -f coordinator4.py
        pkill -u dario -9 -f coordinator4.py
        pkill -9 -f client_eris4.py
        pkill -u dario -9 -f client_eris4.py
        pkill -9 -f coordinator4.py
        pkill -u dario -9 -f coordinator4.py
        pkill -9 -f client_eris4.py
        pkill -u dario -9 -f client_eris4.py
